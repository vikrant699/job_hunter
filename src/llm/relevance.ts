 ...
 * an anti-hedge instruction, a recall-safety floor, and three worked examples.
 *
 ...
 * (analysis and the four sub-scores are optional; matchScore, the dealBreaker fields, and reason are required).
 */
export const RELEVANCE_PROMPT = `You are screening job postings for ONE specific candidate. Your score decides what they see: high scores are shown first as strong matches, low scores are hidden. The candidate is ACTIVELY job-hunting and is trusting you to surface every real match — missing a genuine fit is worse than showing a borderline one. Be decisive and use the FULL 0..1 range.
