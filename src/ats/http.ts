...
async function fetchOk(url: string, init: RequestInit, provider: string): Promise<Response> {
  ...
  if (!res.ok) {
    ...
    // Scan the FULL body for a WAF/block-page signature before atsHttpError cuts
    // it to a 200-char snippet: CloudFront/Akamai pages carry their markers
    // ("Generated by cloudfront", "Access Denied" + "permission") past that cut,
    // so the stored error otherwise looks board-shaped and the scheduler charges
    // the row a consecutive failure for an edge refusing the moment (run 37 did
    // exactly that to qualcomm + hsbc). An edge-challenge error classifies as an
    // infrastructure fault: retried inline, deferred to end-of-run, never charged.
