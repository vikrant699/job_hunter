  ...
  async listPostings(company: AdapterCompany): Promise<NormalizedPosting[]> {
    ...
    return paginate<NormalizedPosting>({
      ...
      fetchPage: async (_offset, page) => {
        const pageNum = page + 1; // API is 1-based
        const url = `https://io.spire2grow.com/ies/v1/p/requisition/_search?page=${pageNum}&size=${SIZE}&selectedSortOrder=desc&selectedSortField=postedOn`;
        ...
        return { items: rows.map((j) => normalizeSpire2Grow(company, j)), total: null };
      },
    });
  },
};
