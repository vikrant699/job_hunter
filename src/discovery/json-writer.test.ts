...
const E = (source: string, slug: string, name: string): RegistryEntry => ({
  name, careers_url: "https://x/" + slug, source: source as RegistryEntry["source"],
...
test("upsertRegistry adds entries and writes them sorted by source then slug", () => {
  ...
  const arr = JSON.parse(readFileSync(f, "utf8")) as RegistryEntry[];
